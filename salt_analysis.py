...
reformdf = calc2.dataframe(variable_list=[], all_vars=True)
reformdf.loc[:, ['RECID', 's006']] # this has the initial 2021 weights!
# what is the weighted value of the SALT deduction for millionaires??
((reformdf.c00100 > 1e6) * reformdf.s006 * reformdf.c18300).sum() / 1e9
(reformdf.s006 * reformdf.c18300).sum() / 1e9

basedf.to_parquet(SCRATCHDIR + 'base2021.parquet', engine='pyarrow')
reformdf.to_parquet(SCRATCHDIR + 'reform2021.parquet', engine='pyarrow')
